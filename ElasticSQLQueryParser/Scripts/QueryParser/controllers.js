...
elasticsearchSqlApp.controller('MainController', function ($scope, $http, $sce,$compile) {
  ...
  function loadOptionsFromStorageOrDefault () {
    ...
    $http.get($scope.url + "Check").success(function (data) {
        $http.get($scope.url + "Node/" + data.name).success(function (nodeData) {
...
function updateWithScrollIfNeeded (query) {
  ...
  return query;
    ...
    query = updateWithScrollIfNeeded(query);
		$http.post($scope.url + "Sql", { Query : query })
		.success(function(data, status, headers, config) {
          var handler = ResultHandlerFactory.create(data,$scope.config.isFlat);
          updateDescription(handler);
          if(handler.isScroll){
	...
	$scope.explain = function() {
        ...
        var query = window.editor.getValue();
		$http.post($scope.url + "Explain", { Query : query })
		.success(function(data, status, headers, config) {
					 $scope.resultExplan = true;
	...
	function getUrl() {
		...
		if(url == undefined) {
			if(location.protocol == "file") {
				...
				url = location.protocol+'//' + location.hostname + (location.port ? ':'+location.port : '') + "/QueryParser/";
			}
		}
